...
class CasinoBot:
    def __init__(self):
        ...
        self.muted_users = {}  # Новое: мьюты пользователей {user_id: timestamp}
        self.admin_logs = []
        ...
        # Новое: Ивент Хеллоуин
        self.halloween_event = {
            "active": True,
            "start_time": time.time(),
            "end_time": time.time() + 30 * 24 * 3600  # 30 дней
        }
        
        # Новое: Лимит привилегии Хеллоуина
        self.limit_privileges = {
            "spooky": {"multiplier": 1.5, "title": "👻 Spooky 👻", "cost": 150, "event": "🎃 Halloween 🎃"},
            "scary": {"multiplier": 2.0, "title": "🦇 Scary 🦇", "cost": 250, "event": "🎃 Halloween 🎃"},
            "pumpkin_emperor": {"multiplier": 3.0, "title": "🎃 Pumpkin Emperor 🎃", "cost": 350, "event": "🎃 Halloween 🎃"},
            "vampire": {"multiplier": 3.5, "title": "🧛 Vampire 🧛", "cost": 500, "event": "🎃 Halloween 🎃"},
            "halloween_king": {"multiplier": 5.0, "title": "🎃👑 Halloween King 👑🎃", "cost": 700, "event": "🎃 Halloween 🎃"}
        }
        
        # Новое: Лимит титулы Хеллоуина
        self.limit_titles = {
            "halloween": {"title": "🎃 Halloween 🎃", "cost": 50, "event": "🎃 Halloween 🎃"},
            "sweet": {"title": "🍬 Сладкий 🍬", "cost": 70, "event": "🎃 Halloween 🎃"},
            "ghost": {"title": "👻 Призрак 👻", "cost": 100, "event": "🎃 Halloween 🎃"}
        }
    ...
    def save_data(self):
        try:
            data = {
                ...
                'halloween_event': self.halloween_event
            }
    ...
    def load_data(self):
        try:
            if os.path.exists(self.data_file):
                ...
                self.halloween_event = data.get('halloween_event', self.halloween_event)
                
                self.users = {int(k): v for k, v in self.users.items()}
                self.muted_users = {int(k): v for k, v in self.muted_users.items()}
    ...
    def init_user(self, user_id: int, username: str):
        if user_id not in self.users:
            ...
            self.users[user_id] = {
                ...
                "limit_privileges": [],
                "limit_titles": [],
                "active_limit_privilege": None,
                "active_limit_title": None
            }
            if is_creator:
    ...
    def check_privilege_bonus(self, user_id: int, base_win: int) -> int:
        ...
        # Новое: проверка лимит привилегии
        if user["active_limit_privilege"]:
            limit_priv_info = self.limit_privileges[user["active_limit_privilege"]]
            bonus_multiplier *= limit_priv_info["multiplier"]
    ...
    def send_club_message(self, user_id: int, message: str, context: CallbackContext = None):
        ...
        # Новое: добавление титула к имени
        display_name = username
        if user["active_limit_title"]:
            title_info = self.limit_titles[user["active_limit_title"]]
            display_name = f"{title_info['title']} {username}"
        ...
        formatted_message = f"💬 **{club_name}**\n[{rank_name}] {display_name}:\n{message}"
    ...
    # НОВЫЕ МЕТОДЫ ДЛЯ ХЕЛЛОУИНА

    def start_halloween_king_event(self, user_id: int):
        user = self.users[user_id]
        if user["active_event"]:
            return "❌ Вы уже выполняете задание! Закончите текущее задание перед началом нового."
        
        user["active_event"] = "halloween_king"
        user["event_progress"] = {"candies_collected": 0}
        self.save_data()
        
        return ("🎃 **Хеллоуинский Король 👑** 🎃\n\n"
                "Тинь-Тинь-Тинь. Привет! Юный пользователь! Хочешь заработать конфет 🍬?\n\n"
                "Твоя задача: сыграть во все 4 игры (/play [ставка] [номер игры]), и возможно что в одной из игр ты можешь получить конфет при выигрыше! 🎃\n\n"
                "🎯 Шанс получить конфеты: 35% при выигрыше\n"
                "🍬 Цель: собрать 50 конфет\n"
                "💰 Награда: возможность купить эксклюзивные привилегии в /candyshop")

    def start_witch_event(self, user_id: int):
        user = self.users[user_id]
        if user["active_event"]:
            return "❌ Вы уже выполняете задание! Закончите текущее задание перед началом нового."
        
        user["active_event"] = "witch"
        user["event_progress"] = {
            "luck_wins": 0,
            "other_wins": {1: 0, 2: 0, 3: 0}
        }
        self.save_data()
        
        return ("🎃👻 **Ведьма** 👻🎃\n\n"
                "Хм-Хм. Здравствуй, Юный пользователь! Я ведьма, и у меня нету ингредиентов для сварки волшебного зелья, не поможешь ли ты мне найти Ингредиенты за Конфеты?\n\n"
                "Твоя задача:\n"
                "• Выиграть 2 раза в игре 🍀 Удача\n"
                "• Выиграть по 1 разу в играх: 🎡 Рулетка, 🎯 Координаты, 🪙 Монетка\n\n"
                "🍬 Награда: 30 Конфет")

    def add_halloween_candies(self, user_id: int, game_id: int, won: bool):
        user = self.users[user_id]
        
        if not user["active_event"] == "halloween_king":
            return False
        
        if won and random.random() <= 0.35:  # 35% шанс
            candies_to_add = 10
            user["candies"] += candies_to_add
            user["event_progress"]["candies_collected"] += candies_to_add
            
            # Проверка завершения задания
            if user["event_progress"]["candies_collected"] >= 50:
                user["active_event"] = None
                user["event_cooldown"] = time.time() + 7200  # 2 часа кд
                self.save_data()
                return "completed"
            
            self.save_data()
            return candies_to_add
        
        return False

    def update_witch_progress(self, user_id: int, game_id: int, won: bool):
        user = self.users[user_id]
        
        if not user["active_event"] == "witch":
            return False
        
        if not won:
            return False
        
        progress = user["event_progress"]
        
        if game_id == 4:  # Игра Удача
            progress["luck_wins"] += 1
        elif game_id in [1, 2, 3]:  # Другие игры
            progress["other_wins"][game_id] += 1
        
        # Проверка завершения задания
        luck_completed = progress["luck_wins"] >= 2
        other_completed = all(wins >= 1 for wins in progress["other_wins"].values())
        
        if luck_completed and other_completed:
            user["candies"] += 30
            user["active_event"] = None
            user["event_cooldown"] = time.time() + 7200  # 2 часа кд
            self.save_data()
    ...
    def buy_limit_privilege(self, user_id: int, privilege_name: str):
        user = self.users[user_id]
        
        if privilege_name not in self.limit_privileges:
            return "❌ Такой лимит привилегии не существует"
        
        privilege_info = self.limit_privileges[privilege_name]
        
        if user["candies"] < privilege_info["cost"]:
            return f"❌ Недостаточно конфет. Нужно: {privilege_info['cost']} 🍬"
        
        if privilege_name in user["limit_privileges"]:
            return "❌ У вас уже есть эта лимит привилегия"
        
        user["candies"] -= privilege_info["cost"]
        user["limit_privileges"].append(privilege_name)
        self.save_data()
        
        return (f"🎉 Поздравляем с покупкой Лимит Привелегии: {privilege_info['title']} !\n\n"
                f"Для применения привилегии введите команду — /limitpr {privilege_name}")

    def buy_limit_title(self, user_id: int, title_name: str):
        user = self.users[user_id]
        
        if title_name not in self.limit_titles:
            return "❌ Такой лимит титул не существует"
        
        title_info = self.limit_titles[title_name]
        
        if user["candies"] < title_info["cost"]:
            return f"❌ Недостаточно конфет. Нужно: {title_info['cost']} 🍬"
        
        if title_name in user["limit_titles"]:
            return "❌ У вас уже есть этот лимит титул"
        
        user["candies"] -= title_info["cost"]
        user["limit_titles"].append(title_name)
        self.save_data()
        
        return (f"🎉 Поздравляем с покупкой Лимит Титула: {title_info['title']} !\n\n"
                f"Для применения титула введите команду — /limittitl {title_name}")

    def set_limit_privilege(self, user_id: int, privilege_name: str):
        user = self.users[user_id]
        
        if privilege_name not in user["limit_privileges"]:
            return "❌ У вас нет этой лимит привилегии"
        
        user["active_limit_privilege"] = privilege_name
        self.save_data()
        
        privilege_info = self.limit_privileges[privilege_name]
        return f"✅ Лимит привилегия установлена: {privilege_info['title']}"

    def set_limit_title(self, user_id: int, title_name: str):
        user = self.users[user_id]
        
        if title_name not in user["limit_titles"]:
            return "❌ У вас нет этого лимит титула"
        
        user["active_limit_title"] = title_name
        self.save_data()
        
        title_info = self.limit_titles[title_name]
        return f"✅ Лимит титул установлен: {title_info['title']}"

    def mute_user(self, user_id: int, target_id: int, minutes: int):
        if target_id not in self.users:
            return "❌ Пользователь не найден"
        
        if self.is_creator(target_id):
            return "❌ Нельзя замутить создателя"
        
        mute_duration = minutes * 60
        self.muted_users[target_id] = time.time() + mute_duration
        self.save_data()
        
        target_username = self.users[target_id]["username"] or str(target_id)
        return f"✅ Пользователь @{target_username} замьючен на {minutes} минут"

    def unmute_user(self, user_id: int, target_id: int):
        if target_id in self.muted_users:
            del self.muted_users[target_id]
            self.save_data()
            
            target_username = self.users[target_id]["username"] or str(target_id)
            return f"✅ Пользователь @{target_username} размьючен"
        else:
...
def help_cmd(update: Update, context: CallbackContext):
    ...
    if bot_data.is_muted(user_id):
        update.message.reply_text("❌ Вы в муте и не можете использовать команды.")
        return
...
🎃 **Хеллоуин Ивент:**
/event - Начать ивент Хеллоуин
/candyshop - Магазин конфет
/limitpr [привилегия] - Установить лимит привилегию
/limittitl [титул] - Установить лимит титул
...
def transfer(update: Update, context: CallbackContext):
    ...
    if bot_data.is_muted(target_id):
        update.message.reply_text("❌ Нельзя переводить пользователю в муте")
        return
...
def start(update: Update, context: CallbackContext):
    ...
    if bot_data.is_muted(user_id):
        update.message.reply_text("❌ Вы в муте и не можете использовать бота.")
        return
    ...
    # Новое: информация о конфетах
    user_data = bot_data.users[user_id]
    candies_info = f"🍬 Конфеты: {user_data['candies']}" if user_data['candies'] > 0 else ""
    
    # Новое: информация об ивенте
    event_info = ""
    if bot_data.halloween_event["active"]:
        event_info = "\n\n❗Событие (ивент)❗\n\n🎃 Хеллоуин 🎃"
...
def play(update: Update, context: CallbackContext):
    ...
    # Новое: обработка ивентов Хеллоуина
    event_result = ""
    if user["active_event"] == "halloween_king" and won:
        candies_result = bot_data.add_halloween_candies(user_id, game_id, won)
        if candies_result == "completed":
            event_result = "\n\n🎃 **Хеллоуинский Король 👑** 🎃\nПоздравляю! Ты выполнил задание, и получил 50 конфет 🍬!"
        elif candies_result:
            event_result = f"\n\n🎃 +{candies_result} конфет 🍬 (Хеллоуинское задание)"
    
    elif user["active_event"] == "witch" and won:
        witch_result = bot_data.update_witch_progress(user_id, game_id, won)
        if witch_result == "completed":
            event_result = "\n\n🎃👻 **Ведьма** 👻🎃\nСпасибо! Ты помог мне с поиском ингредиентов! Держи — 30 Конфет 🍬"
        elif witch_result == "progress":
            progress = user["event_progress"]
            if game_id == 4 and progress["luck_wins"] == 2:
                event_result = "\n\n🎃👻 **Ведьма** 👻🎃\nПоздравляю! Ты нашел один из ингредиентов!"
            elif game_id in [1, 2, 3] and progress["other_wins"][game_id] == 1:
                event_result = "\n\n🎃👻 **Ведьма** 👻🎃\nПоздравляю! Ты нашел один из ингредиентов!"
...
def button_handler(update: Update, context: CallbackContext):
    ...
    if bot_data.is_muted(user_id):
        query.edit_message_text("❌ Вы в муте и не можете использовать команды.")
        return
    ...
    # Новое: обработка кнопок Хеллоуина
    elif data.startswith("event_"):
        event_type = data.split("_")[1]
        user_id = query.from_user.id
        
        if event_type == "halloween_king":
            result = bot_data.start_halloween_king_event(user_id)
            keyboard = [
                [InlineKeyboardButton("✅ Начать выполнение", callback_data="start_halloween_king")],
                [InlineKeyboardButton("❌ Извини, не хочу", callback_data="cancel_event")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            query.edit_message_text(result, reply_markup=reply_markup)
        
        elif event_type == "witch":
            result = bot_data.start_witch_event(user_id)
            keyboard = [
                [InlineKeyboardButton("✅ Начать выполнение", callback_data="start_witch")],
                [InlineKeyboardButton("❌ Извини, давай позже", callback_data="cancel_event")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            query.edit_message_text(result, reply_markup=reply_markup)
    
    elif data == "start_halloween_king":
        user_id = query.from_user.id
        user = bot_data.users[user_id]
        user["active_event"] = "halloween_king"
        user["event_progress"] = {"candies_collected": 0}
        bot_data.save_data()
        query.edit_message_text("🎃 Вы начали выполнение задания Хеллоуинского Короля! Играйте в игры (/play) чтобы собирать конфеты!")
    
    elif data == "start_witch":
        user_id = query.from_user.id
        user = bot_data.users[user_id]
        user["active_event"] = "witch"
        user["event_progress"] = {"luck_wins": 0, "other_wins": {1: 0, 2: 0, 3: 0}}
        bot_data.save_data()
        query.edit_message_text("🎃👻 Вы начали выполнение задания Ведьмы! Выполняйте требования задания чтобы получить награду!")
    
    elif data == "cancel_event":
        query.edit_message_text("❌ Вы отменили выполнение задания.")
    
    elif data.startswith("candy_buy_"):
        item_type = data.split("_")[2]
        item_name = data.split("_")[3]
        user_id = query.from_user.id
        
        if item_type == "privilege":
            result = bot_data.buy_limit_privilege(user_id, item_name)
        elif item_type == "title":
            result = bot_data.buy_limit_title(user_id, item_name)
    ...
    elif data.startswith("event_continue_"):
        query.edit_message_text("✅ Продолжайте выполнение задания!")
...
def stats(update: Update, context: CallbackContext):
    ...
    # Новое: информация о лимит привилегиях и титулах
    limit_privileges_text = "Нет"
    if user["limit_privileges"]:
        limit_privileges_text = "\n".join([f"- {bot_data.limit_privileges[priv]['title']} ({bot_data.limit_privileges[priv]['event']})" for priv in user["limit_privileges"]])
    
    limit_titles_text = "Нет"
    if user["limit_titles"]:
        limit_titles_text = "\n".join([f"- {bot_data.limit_titles[title]['title']} ({bot_data.limit_titles[title]['event']})" for title in user["limit_titles"]])
    
    active_limit_priv = "Не установлена"
    if user["active_limit_privilege"]:
        active_limit_priv = bot_data.limit_privileges[user["active_limit_privilege"]]["title"]
    
    active_limit_title = "Не установлен"
    if user["active_limit_title"]:
        active_limit_title = bot_data.limit_titles[user["active_limit_title"]]["title"]
    ...
    update.message.reply_text(
        ...
        f"💎 **ЛИМИТ ПРИВЕЛЕГИИ** 💎\n"
        f"Активная: {active_limit_priv}\n"
        f"Доступные:\n{limit_privileges_text}\n\n"
        f"💎 **ЛИМИТ ТИТУЛЫ** 💎\n"
        f"Активный: {active_limit_title}\n"
        f"Доступные:\n{limit_titles_text}",
        parse_mode='Markdown'
    )
...
def creatorcmd(update: Update, context: CallbackContext):
    ...
    commands_list = [
        ...
        "/mute [ID] [минуты] - выдать мут пользователю",
        "",
        "🎮 УПРАВЛЕНИЕ ИГРАМИ:",
...
def userinfo(update: Update, context: CallbackContext):
    ...
    response += f"🍬 Конфеты: {user_data['candies']} 🍬\n"
    ...
    response += f"🔇 Статус мута: {'Да' if target_id in bot_data.muted_users else 'Нет'}"
...
def logs(update: Update, context: CallbackContext):
    ...
    for log in reversed(bot_data.admin_logs[-20:]):
        ...
        if log["action"] == "НАЗНАЧЕНИЕ_АДМИНА":
            ...
            action_text = f"замьютил {target} ({log['details']})"
...
def messagefriend(update: Update, context: CallbackContext):
    ...
    if isinstance(result, int):
        try:
            # Новое: добавление титула к имени отправителя
            sender_name = update.effective_user.username or f"Пользователь {user_id}"
            if bot_data.users[user_id]["active_limit_title"]:
                title_info = bot_data.limit_titles[bot_data.users[user_id]["active_limit_title"]]
                sender_name = f"{title_info['title']} {sender_name}"
            
            context.bot.send_message(
                chat_id=result,
                text=f"💌 Сообщение от друга {sender_name}:\n\n{message}"
            )
...
def backup(update: Update, context: CallbackContext):
    ...
    try:
        ...
        update.message.reply_text(
            ...
            f"• 🔇 В муте: {len(bot_data.muted_users)}\n"
...
def globalstats(update: Update, context: CallbackContext):
    ...
    response += f"• 🏆 Всего заработано: {stats['total_earned']}\n"
    response += f"• 🚫 Забанено: {stats['banned_users']}\n"
    response += f"• 🔇 В муте: {len(bot_data.muted_users)}\n"
    response += f"• 🔧 Администраторов: {stats['active_admins']}\n\n"
...
# НОВЫЕ КОМАНДЫ ДЛЯ ХЕЛЛОУИНА

def event_cmd(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    ...
    if user["active_event"]:
        keyboard = [
            [
                InlineKeyboardButton("❌ Отменить выполнение", callback_data=f"event_cancel_{user['active_event']}"),
                InlineKeyboardButton("✅ Продолжить выполнение", callback_data=f"event_continue_{user['active_event']}")
            ]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        event_name = "Хеллоуинское Задание"
        if user["active_event"] == "halloween_king":
            event_name = "Хеллоуинский Король"
        elif user["active_event"] == "witch":
            event_name = "Задание Ведьмы"
        
        update.message.reply_text(
            f"🎃 Вы уже выполняете {event_name}, хотите закончить выполнение?",
            reply_markup=reply_markup
        )
        return
    ...
    # Проверка кд
    current_time = time.time()
    if user["event_cooldown"] and current_time < user["event_cooldown"]:
        remaining = int(user["event_cooldown"] - current_time)
        hours = remaining // 3600
        minutes = (remaining % 3600) // 60
        seconds = remaining % 60
        update.message.reply_text(f"⏰ Следующее задание будет доступно через: {hours:02d}:{minutes:02d}:{seconds:02d}")
        return
    
    # Меню выбора задания
    keyboard = [
        [InlineKeyboardButton("🎃👑 Хеллоуинский Король 👑🎃", callback_data="event_halloween_king")],
        [InlineKeyboardButton("🎃👻 Ведьма 👻🎃", callback_data="event_witch")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    update.message.reply_text(
        ...
        "🎃👑 **Хеллоуинский Король**\n"
        "• Собирайте конфеты в играх\n"
        "• Шанс: 35% при выигрыше\n"
        "• Цель: 50 конфет\n\n"
        "🎃👻 **Ведьма**\n"
        "• Выполняйте требования по победам\n"
        "• Награда: 30 конфет\n"
        "• Перезарядка: 2 часа",
        reply_markup=reply_markup
    )

def candyshop(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    ...
    # Привилегии
    keyboard.append([InlineKeyboardButton("💎 ПРИВЕЛЕГИИ 💎", callback_data="header_privileges")])
    for priv_name, priv_info in bot_data.limit_privileges.items():
        button_text = f"{priv_info['title']} - {priv_info['cost']} 🍬 (x{priv_info['multiplier']})"
        keyboard.append([InlineKeyboardButton(button_text, callback_data=f"candy_buy_privilege_{priv_name}")])
    
    # Титулы
    keyboard.append([InlineKeyboardButton("🎗️ УКРАШЕНИЯ В ПРОФИЛЕ 🎗️", callback_data="header_titles")])
    for title_name, title_info in bot_data.limit_titles.items():
        button_text = f"{title_info['title']} - {title_info['cost']} 🍬"
        keyboard.append([InlineKeyboardButton(button_text, callback_data=f"candy_buy_title_{title_name}")])
    ...
    shop_text = (
        "                      🍭🍬 Candy Shop 🍬🍭\n\n"
        "                          💎 Привелегии 💎\n"
        "1. 👻 Spooky 👻 - x1.5 к выигрышу - 150 🍬\n"
        "2. 🦇 Scary 🦇 - x2 к выигрышу - 250 🍬\n"
        "3. 🎃 Pumpkin Emperor 🎃 - x3 к выигрышу - 350 🍬\n"
        "4. 🧛 Vampire 🧛 - x3.5 к выигрышу - 500 🍬\n"
        "5. 🎃👑 Halloween King 👑🎃 - x5 к выигрышу - 700 🍬\n\n"
        "               🎗️ Украшения в профиле 🎗️\n"
        "1. 🎃 Halloween 🎃 - 50 🍬\n"
        "2. 🍬 Сладкий 🍬 - 70 🍬\n"
        "3. 👻 Призрак 👻 - 100 🍬\n\n"
        f"🍬 Ваши конфеты: {user['candies']}"
    )
    
    update.message.reply_text(shop_text, reply_markup=reply_markup)

def limitpr(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    ...
    if not context.args:
        available_privs = ", ".join(bot_data.users[user_id]["limit_privileges"])
        update.message.reply_text(
            f"👑 Установка лимит привилегии\n\n"
            f"Использование: /limitpr [название привилегии]\n\n"
            f"💎 Ваши привилегии: {available_privs if available_privs else 'Нет'}\n\n"
            f"💡 Доступные привилегии:\n"
            f"• spooky, scary, pumpkin_emperor, vampire, halloween_king"
        )
        return
    
    privilege_name = context.args[0].lower()
    result = bot_data.set_limit_privilege(user_id, privilege_name)
    update.message.reply_text(result)

def limittitl(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    ...
    if not context.args:
        available_titles = ", ".join(bot_data.users[user_id]["limit_titles"])
        update.message.reply_text(
            f"🎗️ Установка лимит титула\n\n"
            f"Использование: /limittitl [название титула]\n\n"
            f"💎 Ваши титулы: {available_titles if available_titles else 'Нет'}\n\n"
            f"💡 Доступные титулы:\n"
            f"• halloween, sweet, ghost"
        )
        return
    
    title_name = context.args[0].lower()
    result = bot_data.set_limit_title(user_id, title_name)
    update.message.reply_text(result)
...
def mute(update: Update, context: CallbackContext):
    ...
    if len(context.args) != 2:
        update.message.reply_text("Использование: /mute [ID пользователя] [минуты]\nПример: /mute 1234567 20")
        return
    ...
    try:
        ...
        update.message.reply_text("❌ Ошибка: ID и минуты должны быть числами")
        return
    ...
    if bot_data.is_creator(target_id):
        update.message.reply_text("❌ Нельзя замутить создателя")
        return
    
    if minutes <= 0 or minutes > 1440:  # Максимум 24 часа
        update.message.reply_text("❌ Минуты должны быть от 1 до 1440 (24 часа)")
        return
    
    result = bot_data.mute_user(user_id, target_id, minutes)
    
    if not bot_data.is_creator(user_id):
        admin_username = update.effective_user.username or str(user_id)
        target_username = bot_data.users[target_id]["username"] or str(target_id)
        bot_data.add_admin_log(user_id, admin_username, "МУТ", target_username, f"{minutes} минут")
...
def unmute(update: Update, context: CallbackContext):
    ...
    if len(context.args) != 1:
        update.message.reply_text("Использование: /unmute [ID пользователя]")
        return
    ...
    result = bot_data.unmute_user(user_id, target_id)
    
    if not bot_data.is_creator(user_id):
        admin_username = update.effective_user.username or str(user_id)
        target_username = bot_data.users[target_id]["username"] or str(target_id)
        bot_data.add_admin_log(user_id, admin_username, "РАЗМУТ", target_username)
...
def main():
    ...
    dispatcher.add_handler(CommandHandler("ccsafe", ccsafe))
    dispatcher.add_handler(CommandHandler("cbuylevel", cbuylevel))
    dispatcher.add_handler(CommandHandler("cadd", cadd))
    dispatcher.add_handler(CommandHandler("cleave", cleave))
    
    # Админские команды создателя
    ...
    # Новые команды Хеллоуина
    dispatcher.add_handler(CommandHandler("event", event_cmd))
    dispatcher.add_handler(CommandHandler("candyshop", candyshop))
    dispatcher.add_handler(CommandHandler("limitpr", limitpr))
    dispatcher.add_handler(CommandHandler("limittitl", limittitl))
