...
class CasinoBot:
    def __init__(self):
        ...
        self.friends_requests = {}  # {user_id: {from_id: timestamp}}
        self.friends = {}  # {user_id: [friend_id1, friend_id2]}
        self.friends_names = {}  # {user_id: {friend_id: "friend_name"}}
        
        # Новая система клубов
        self.clubs = {}  # {club_name: club_data}
        self.club_ranks = {}  # {club_name: {1: "Название", 2: "Название", ...}}
        self.club_messages = {}  # {club_name: [message1, message2]}
        self.club_join_requests = {}  # {club_name: {user_id: timestamp}}
    ...
    def auto_save(self):
        ...
        while True:
            ...
            self.save_data()
            logger.info("💾 Авто-сохранение выполнено")
    ...
    def create_club(self, user_id: int, club_name: str):
        ...
        # Устанавливаем стандартные названия рангов
        ...
        # Добавляем пользователю информацию о клубе
        user["club"] = club_name
        user["club_rank"] = 6  # Создатель
        
        self.save_data()
        return f"✅ Клуб '{club_name}' успешно создан!"

    def club_salary_distribution(self):
        """Распределение зарплаты в клубах"""
        current_time = time.time()
        
        for club_name, club_data in self.clubs.items():
            # Проверяем, прошло ли 2.5 часа
            if current_time - club_data["last_salary_time"] < 9000:  # 2.5 часа в секундах
                continue
            ...
            if salary > 0:
                # Выдаем зарплату всем участникам
                ...
                logger.info(f"💰 Зарплата клуба {club_name}: {salary} каждому участнику")
        
        self.save_data()

    def set_club_rank(self, user_id: int, target_id: int, rank: int):
        """Установить ранг участнику клуба"""
        user = self.users[user_id]
        ...
        # Проверяем, что целевой пользователь в клубе
    ...
    def send_club_message(self, user_id: int, message: str):
        """Отправить сообщение в чат клуба"""
        user = self.users[user_id]
        if "club" not in user or not user["club"]:
            return "❌ Вы не состоите в клубе"
        ...
        # Отправляем всем участникам клуба
        sent_count = 0
        for member_id in self.clubs[club_name]["members"]:
            if member_id != user_id and member_id in self.users:
                try:
                    # В реальной реализации здесь будет отправка через context.bot
                    # context.bot.send_message(chat_id=member_id, text=formatted_message)
                    sent_count += 1
                except Exception as e:
    ...
    def deposit_to_club_safe(self, user_id: int, amount: int, message: str = ""):
        ...
        # Сохраняем в историю сообщений клуба
        if club_name not in self.club_messages:
            self.club_messages[club_name] = []
    ...
    def withdraw_from_club_safe(self, user_id: int, amount: int):
        """Взять деньги из сейфа клуба (только создатель)"""
        user = self.users[user_id]
        if "club" not in user or not user["club"]:
            return "❌ Вы не состоите в клубе"
    ...
    def accept_club_invite(self, user_id: int, club_name: str):
    ...
    def leave_club(self, user_id: int):
        ...
        # Если пользователь - создатель, клуб удаляется
        if club_data["creator"] == user_id:
            # Удаляем клуб
            for member_id in club_data["members"]:
                if member_id in self.users:
    ...
    def update_club_rank_names(self, user_id: int, rank_names: Dict[int, str]):
        """Обновить названия рангов в клубе"""
        user = self.users[user_id]
        if "club" not in user or not user["club"]:
            return "❌ Вы не состоите в клубе"
        ...
        # Проверяем, является ли пользователь создателем
        if club_data["creator"] != user_id:
            return "❌ Только создатель клуба может изменять названия рангов"
        
        # Обновляем названия рангов
        for rank, name in rank_names.items():
            if 1 <= rank <= 5:  # Только ранги 1-5 можно изменять
                self.club_ranks[club_name][rank] = name
        
        self.save_data()
        return "✅ Названия рангов успешно обновлены!"
...
def button_handler(update: Update, context: CallbackContext):
    ...
    if data.startswith("buy_"):
        ...
        if bot_data.accept_friend_request(to_user_id, from_user_id):
        ...
        if bot_data.accept_club_invite(user_id, club_name):
        ...
        if club_name in bot_data.club_join_requests and user_id in bot_data.club_join_requests[club_name]:
            ...
            bot_data.save_data()
            query.edit_message_text(f"❌ Приглашение в клуб '{club_name}' отклонено")
...
def creatorcmd(update: Update, context: CallbackContext):
    ...
    commands_list = [
        ...
        "/testmode - войти в тестовый режим",
...
def addfriend(update: Update, context: CallbackContext):
    ...
    try:
        context.bot.send_message(
            chat_id=target_id,
            text=f"👥 Запрос в друзья от @{update.effective_user.username}\n\nХотите принять?",
            reply_markup=reply_markup
        )
...
def messagefriend(update: Update, context: CallbackContext):
    ...
    if isinstance(result, int):  # friend_id
        try:
            context.bot.send_message(
                chat_id=result,
...
def cchat(update: Update, context: CallbackContext):
    ...
    message = ' '.join(context.args)
    result = bot_data.send_club_message(user_id, message)
    update.message.reply_text(result)
...
def cadd(update: Update, context: CallbackContext):
    ...
    if isinstance(result, int):  # target_id
        # Создаем кнопки для подтверждения
        user = bot_data.users[user_id]
        club_name = user["club"]
        ...
        keyboard = [
            [
                InlineKeyboardButton("✅ Принять", callback_data=f"club_accept_{club_name}"),
        ...
        try:
            context.bot.send_message(
                chat_id=result,
                text=f"🏢 Приглашение в клуб '{club_name}'\n\nХотите присоединиться?",
                reply_markup=reply_markup
            )
...
def testmode_user(update: Update, context: CallbackContext):
    ...
    test_user_id = user_id + 1000000  # Смещаем ID
...
def main():
    ...
    # Запуск распределения зарплат
    salary_thread = threading.Thread(target=bot_data.club_salary_distribution, daemon=True)
    salary_thread.start()
    ...
    logger.info("👥 Система друзей добавлена")
