...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bot_data.creator_id is None:
        ...
        await update.message.reply_text(
            ...
            f"Ваш ID: {user_id}\n\n"
            f"💎 Вы получили бесконечные монеты и специальные команды!\n\n"
            f"📋 Основные команды:\n"
            f"/play [ставка] [игра] - играть\n"
            f"/shop - магазин\n"
            f"/leaderboard - лидеры\n"
            f"/stats - статистика\n"
            f"/wheel - колесо удачи\n"
            f"/author - автор\n\n"
            f"⚙️ Команды создателя:\n"
            f"/givecash [id] [amount]\n"
            f"/setdonate [id] [donate]\n"
            f"/ban [id]\n"
            f"/unban [id]"
        )
    else:
        if bot_data.is_creator(user_id):
            await update.message.reply_text("Добро пожаловать, создатель! 👑")
        else:
            await update.message.reply_text("Добро пожаловать в казино-бот! 🎰")
