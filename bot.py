...
class CasinoBot:
    ...
    def get_bot_stats(self) -> Dict:
        ...
        total_balance = sum(user["balance"] for user in self.users.values() if not self.is_creator(user_id))
        total_earned = sum(user["total_earned"] for user in self.users.values() if not self.is_creator(user_id))
...
async def botstats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for game_id, game_data in game_stats.items():
        ...
        response += f"  {game_name}: {game_data['plays']} игр\n"
...
async def exportdata(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = "📁 ЭКСПОРТ ДАННЫХ ПОЛЬЗОВАТЕЛЕЙ:\n\n"
    for user_id, user_data in bot_data.users.items():
...
async def announce(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"• Всего: {len(bot_data.users)} пользователей"
    )
...
# ... (остальные существующие команды: setgladmin, logs, search, givecash, 
# givedonate, ban, unban, author, play, wheel, shop, button_handler, 
# buy_privilege, leaderboard, stats остаются без изменений)
