...
logging.basicConfig(
    ...
    level=logging.INFO,
    handlers=[
        logging.StreamHandler(),  # Вывод в консоль для Railway
    ]
)
...
class CasinoBot:
    def __init__(self):
        ...
        self.games = {
            ...
            4: {"name": "🎲 Кости", "description": "Бросок двух костей - сумма 7 или 11 для выигрыша"}
        }
        self.privileges = {
    ...
    def init_user(self, user_id: int, username: str):
        if user_id not in self.users:
            self.users[user_id] = {
                ...
                "play_coins": 0,
                "privilege": None,
                "available_privileges": [],  # Новое поле для хранения всех доступных привилегий
                "exclusive_donate": None,
                "available_donates": [],     # Новое поле для хранения всех доступных донатов
                "total_earned": 0,
                "games_played": 0,
                "win_streak": 0,
                "last_win": False,
    ...
    def spin_wheel(self, user_id: int) -> str:
        ...
        # Добавляем привилегию в список доступных, если её там нет
        if won_privilege not in user["available_privileges"]:
            user["available_privileges"].append(won_privilege)
        
        # Автоматически надеваем выигранную привилегию
        user["privilege"] = won_privilege
        
        privilege_info = self.privileges[won_privilege]
        return f"🎡 Поздравляем! Вы выиграли: {privilege_info['title']}!\nБонус: +{int((privilege_info['bonus'] - 1) * 100)}% к выигрышам\n\nПривилегия автоматически надета! Используйте /replace для смены."
    ...
    try:
        ...
        if bot_data.creator_id is None:
            ...
            await update.message.reply_text(
                f"🎉 Поздравляем! Вы стали создателем бота!\n"
                f"👤 Ваш ID: `{user_id}`\n\n"
                f"💎 Вы получили бесконечные монеты и PlayCoin!\n\n"
                f"📋 Основные команды:\n"
                ...
                f"/replace [привилегия/донат] - сменить привилегию или донат\n\n"
                ...
                f"/announce [текст] - рассылка всем пользователям\n"
            ...
            elif user_id in bot_data.users and bot_data.users[user_id].get("is_admin", False):
                await update.message.reply_text(
...
# НОВАЯ КОМАНДА: Смена привилегии или доната
async def replace(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not context.args:
            # Показываем доступные привилегии и донаты
            available_privileges = []
            for priv in user.get("available_privileges", []):
                if priv in bot_data.privileges:
                    available_privileges.append(bot_data.privileges[priv]["title"])
            
            available_donates = []
            for donate in user.get("available_donates", []):
                if donate in bot_data.exclusive_donates:
                    available_donates.append(f"{donate} - {bot_data.exclusive_donates[donate]['description']}")
            
            current_privilege = "Нет" if not user["privilege"] else bot_data.privileges[user["privilege"]]["title"]
            current_donate = "Нет" if not user["exclusive_donate"] else f"{user['exclusive_donate']} - {bot_data.exclusive_donates[user['exclusive_donate']]['description']}"
            
            response = (
                f"🔄 Смена привилегий и донатов\n\n"
                f"👑 Текущая привилегия: {current_privilege}\n"
                f"💎 Текущий донат: {current_donate}\n\n"
                f"📂 Доступные привилегии:\n"
            )
            
            if available_privileges:
                for i, priv in enumerate(available_privileges, 1):
                    response += f"{i}. {priv}\n"
            else:
                response += "Нет доступных привилегий\n"
            
            response += f"\n🎁 Доступные донаты:\n"
            if available_donates:
                for i, donate in enumerate(available_donates, 1):
                    response += f"{i}. {donate}\n"
            else:
                response += "Нет доступных донатов\n"
            
            response += f"\n💡 Использование:\n/replace [название]\nПример: /replace bronze\n/replace TITAN"
            
            await update.message.reply_text(response)
            return
        
        target_name = context.args[0].upper() if context.args[0].upper() in bot_data.exclusive_donates else context.args[0].lower()
        
        # Проверяем привилегии
        if target_name in bot_data.privileges:
            if target_name in user.get("available_privileges", []):
                user["privilege"] = target_name
                privilege_info = bot_data.privileges[target_name]
                await update.message.reply_text(f"✅ Привилегия изменена на: {privilege_info['title']}")
            else:
                await update.message.reply_text(f"❌ У вас нет привилегии {target_name}. Приобретите её в магазине /shop")
        
        # Проверяем донаты
        elif target_name in bot_data.exclusive_donates:
            if target_name in user.get("available_donates", []):
                user["exclusive_donate"] = target_name
                donate_info = bot_data.exclusive_donates[target_name]
                await update.message.reply_text(f"✅ Донат изменен на: {target_name}\n{donate_info['description']}")
            else:
                await update.message.reply_text(f"❌ У вас нет доната {target_name}. Получите его у администратора.")
        
        else:
            await update.message.reply_text("❌ Неверное название. Используйте /replace для просмотра доступных вариантов.")
            
    except Exception as e:
        logger.error(f"Error in replace command: {e}")
        await update.message.reply_text("❌ Произошла ошибка при смене привилегии/доната.")
...
async def announce(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    announcement = f"📢 СООБЩЕНИЕ ОТ ВЛАДЕЛЬЦА 📢\n\n{message_text}\n\n💎 Удачи в играх! 🎰"
    
    # Логируем действие
    admin_username = update.effective_user.username or str(user_id)
    bot_data.add_admin_log(user_id, admin_username, "РАССЫЛКА", "", f"Текст: {message_text[:50]}...")
    
    # Отправляем сообщение всем пользователям
    sent_count = 0
    failed_count = 0
    
    for target_id, user_data in bot_data.users.items():
        try:
            if not bot_data.is_banned(target_id):
                await context.bot.send_message(chat_id=target_id, text=announcement)
                sent_count += 1
...
async def global_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    active_users = len([uid for uid, data in bot_data.users.items() if data["games_played"] > 0])
    total_balance = sum(data["balance"] for uid, data in bot_data.users.items() if not bot_data.is_creator(uid))
    total_earned = sum(data["total_earned"] for uid, data in bot_data.users.items() if not bot_data.is_creator(uid))
    total_games = sum(data["games_played"] for data in bot_data.users.values())
    banned_users = len(bot_data.banned_users)
    admin_users = len([data for data in bot_data.users.values() if data.get("is_admin", False)])
    ...
    response = (
        ...
        f"💰 Общий баланс системы: {total_balance:,} монет\n"
        f"🏆 Всего заработано: {total_earned:,} монет\n"
        f"🎯 Сыграно игр: {total_games}\n\n"
        f"📊 Средние показатели:\n"
        f"• {total_balance//max(1,active_users):,} монет на игрока\n"
        f"• {total_games//max(1,active_users):,} игр на игрока\n"
        f"• {total_earned//max(1,total_games):,} монет за игру"
    )
...
async def logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for log in reversed(bot_data.admin_logs[-20:]):  # Последние 20 записей
        ...
        elif log["action"] == "РАССЫЛКА":
            action_text = f"сделал рассылку: {log['details']}"
...
async def givedonate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Добавляем донат в список доступных, если его там нет
    if donate_name not in bot_data.users[target_id].get("available_donates", []):
        bot_data.users[target_id].setdefault("available_donates", []).append(donate_name)
...
async def play(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 2:
        await update.message.reply_text(
            ...
            "2. 🎯 Координаты\n"
            "3. 🪙 Монетка\n"
            "4. 🎲 Кости (сумма 7 или 11 для выигрыша)"
        )
        return
    ...
    if game_id == 1:
    ...
    elif game_id == 4:  # НОВАЯ ИГРА: 🎲 Кости
        ...
        if total in [7, 11]:
            win_amount = bet * 3
            won = True
            result_text = f"🎉 Кости: {dice1} + {dice2} = {total}! Выигрыш 3x! +{win_amount} монет"
        elif total in [2, 3, 12]:
            win_amount = 0
            won = False
            result_text = f"❌ Кости: {dice1} + {dice2} = {total}. Критический провал!"
        else:
            win_amount = bet  # Возврат ставки
            won = True
            result_text = f"➡️ Кости: {dice1} + {dice2} = {total}. Ничья - ставка возвращена"
...
async def shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for priv_id, priv_info in bot_data.privileges.items():
        # Проверяем, есть ли привилегия у пользователя
        has_privilege = priv_id in user.get("available_privileges", [])
        ...
        if has_privilege:
            button_text = f"{status} {title} - УЖЕ КУПЛЕНА (+{bonus}%)"
    ...
    current_priv = "Нет"
    if user["privilege"]:
        current_priv = bot_data.privileges[user["privilege"]]["title"]
    ...
    await update.message.reply_text(
        ...
        f"👑 Текущая привилегия: {current_priv}\n"
        f"📂 Купленные привилегии: {len(user.get('available_privileges', []))}\n\n"
        f"💡 Используйте /replace для смены привилегии",
...
async def buy_privilege(user_id: int, priv_id: str, query):
    ...
    # Проверяем, есть ли уже привилегия
    has_privilege = priv_id in user.get("available_privileges", [])
    ...
    if not has_privilege:
        ...
        # Добавляем привилегию в список доступных
        user.setdefault("available_privileges", []).append(priv_id)
    
    # Надеваем привилегию (делаем текущей)
    user["privilege"] = priv_id
    
    if has_privilege:
        await query.edit_message_text(
            f"🔄 Привилегия активирована!\n"
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    available_privileges = len(user.get("available_privileges", []))
    available_donates = len(user.get("available_donates", []))
    ...
    await update.message.reply_text(
        ...
        f"💰 Баланс: {'∞' if bot_data.is_creator(user_id) else user['balance']}\n"
        f"🎯 PlayCoin: {'∞' if bot_data.is_creator(user_id) else user['play_coins']}\n"
        f"👑 Текущая привилегия: {privilege_title}\n"
        f"💎 Текущий донат: {donate_info}\n"
        f"📂 Доступно привилегий: {available_privileges}\n"
        f"🎁 Доступно донатов: {available_donates}\n"
        f"🎮 Сыграно игр: {user['games_played']}\n"
        f"🏆 Всего заработано: {user['total_earned']}\n"
        f"🔥 Текущая серия побед: {user['win_streak']}\n\n"
        f"💡 Используйте /replace для смены привилегии или доната",
        parse_mode='Markdown'
    )
...
def main():
    ...
    try:
        ...
        application.add_handler(CommandHandler("replace", replace))  # Новая команда
